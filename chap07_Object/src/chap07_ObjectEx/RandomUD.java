...
public class RandomUD {
		...
		void start() {
			...
			// 1. 랜덤한 수 만듦
			this.num = (int) (Math.random() * 100) + 1;   //int.num을 this.num으로 바꿔줌
			System.out.println(num);
	
			// 2. 수를 입력받음
			//int usernum;  >  바로 필드에 있는 usernum으로 접근
		...
		//유저 값을 들고옴
		private int checkUserGuess () {    //return할 값을 잘 모를 때는 임의로 void로 작성해뒀다가  return값을 정하면 int로 바꿈
			Scanner input = new Scanner(System.in);
